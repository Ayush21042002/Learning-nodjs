...
async function getTodos(){
    ...
    for(let entry of todos){
        if(entry.done){
            document.querySelector('#list').innerHTML += `
            <li><input type="checkbox" class="checkBox" id=${entry.id} onclick="changeStatus(this)" checked>
            <label>${entry.task}</label></li>`;
        }
        else {
            document.querySelector("#list").innerHTML += `
            <li><input type="checkbox" class="checkBox" id=${entry.id} onclick="changeStatus(this)" >
            <label>${entry.task}</label></li>        
            `;
        }
    }
...
addButton.onclick = async function addNewTodo(event){
    ...
    const resp = await fetch('/add', {
        ...
        body: JSON.stringify({task,done,due: '2020-05-13'})
    });
    ...
    if(resp.status==400){
        document.getElementById("error-message").innerHTML = "Seems you have not entered any task";
    }
}
